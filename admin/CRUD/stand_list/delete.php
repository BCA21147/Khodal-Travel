...
if (isset($_POST['delete_id'])) {
    ...
    $query = "DELETE coupon.* FROM coupon INNER JOIN trip WHERE (trip.boarding_point LIKE '%[$delete_id]|[%%]%' OR trip.dropping_point LIKE '%[$delete_id]|[%%]%') AND LOCATE(CONCAT('[',trip.id,']'),trip);";
    $query .= "DELETE FROM `stand` WHERE `id` = '$delete_id';";
    $query .= "DELETE FROM `trip` WHERE boarding_point LIKE '%[$delete_id]|[%%]%' or dropping_point LIKE '%[$delete_id]|[%%]%';";
...
if (isset($_GET['id'])) {
    ...
    $result_stand = mysqli_query($con, $query);
    $query = "SELECT * FROM `trip` WHERE boarding_point LIKE '%[$id]|[%%]%' or dropping_point LIKE '%[$id]|[%%]%';";
    $result_trip = mysqli_query($con, $query);
    $query = "SELECT coupon.* FROM coupon INNER JOIN trip WHERE (trip.boarding_point LIKE '%[$id]|[%%]%' OR trip.dropping_point LIKE '%[$id]|[%%]%') AND LOCATE(CONCAT('[',trip.id,']'),trip) GROUP BY coupon.id;";
    $result_coupon = mysqli_query($con, $query);
    
    if ($result_stand && $result_trip && $result_coupon) {
        $stand = mysqli_fetch_assoc($result_stand);
        ?>
        <div class='modal-content'>
            <div class='modal-header'>
                <h5 class='modal-title' id='exampleModalLabel'>DELETE STAND</h5>
            ...
            <div class='modal-body'>
                ...
                <div class='py-2'>
                    <div class='py-1 px-3 display-5' style='background-color: rgba(108, 117, 125,.3);font-weight: bold'>Stand
                    </div>
            ...
            <div class='modal-footer'>
                ...
                <button type='button' class='btn btn-danger' data-dismiss='modal'
                    onclick='stand_delete_data(<?php echo $stand['id']; ?>)'>Confirm</button>
            </div>
        </div>
        <?php
    }

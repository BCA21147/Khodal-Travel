...
if (isset($_POST['delete_id'])) {
    $query = "SELECT * FROM `vehical` WHERE id = $delete_id";
    $result = mysqli_query($con, $query);
    print_r($result);
    if ($result) {
        $vehical = mysqli_fetch_assoc($result);
        foreach (unserialize($vehical['images']) as $key => $value) {
            if (file_exists("../../../Images/$value")) {
                unlink("../../../Images/$value");
            }
        }
        $query = "DELETE coupon.* FROM coupon INNER JOIN trip WHERE trip.vehical_list LIKE '%[$delete_id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip);";
        $query .= "DELETE FROM `vehical` WHERE `id` = '$delete_id';";
        $query .= "DELETE FROM `trip` WHERE vehical_list LIKE '%[$delete_id]|[%%]%';";
...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM `trip` WHERE vehical_list LIKE '%[$id]|[%%]%';";
    $result_trip = mysqli_query($con, $query);
    $query = "SELECT coupon.* FROM coupon INNER JOIN trip WHERE trip.vehical_list LIKE '%[$id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip) GROUP BY coupon.id;";
    $result_coupon = mysqli_query($con, $query);
    if ($result_vehical && $result_trip && $result_coupon) {
        $vehical = mysqli_fetch_assoc($result_vehical);
        $image = unserialize($vehical['images']);
        ?>
        <div class='modal-content'>
            <div class='modal-header'>
                <h5 class='modal-title' id='exampleModalLabel'>DELETE VEHICAL</h5>
                        ...
                        <table class='table table-hover table-bordered'>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <?php echo $vehical['company']; ?>
                                    </td>
                                    <td>
                                        <?php echo explode('|', $vehical['fleet_type'])[1]; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['reg_no']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['eng_no']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['model_no']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['chassis_no']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['owner']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['owner_mobile']; ?>
                                    </td>
                                    <td>
                                        <?php echo $vehical['status']; ?>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class='modal-footer'>
                ...
                <button type='button' class='btn btn-danger' data-dismiss='modal'
                    onclick='vehical_delete_data(<?php echo $vehical['id']; ?>)'>Confirm</button>
            </div>
        </div>
        <?php
    }

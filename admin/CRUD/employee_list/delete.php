...
if (isset($_POST['delete_id'])) {
    $query = "SELECT * FROM `employee` WHERE id = $delete_id";
    $result = mysqli_query($con, $query);
    if ($result) {
        $employee = mysqli_fetch_assoc($result);
        if (file_exists("../../../Images/" . $employee['profile_image'])) {
            unlink("../../../Images/" . $employee['profile_image']);
        }
        if (file_exists("../../../Images/" . $employee['id_image'])) {
            unlink("../../../Images/" . $employee['id_image']);
        }
        $query = "DELETE coupon.* FROM coupon INNER JOIN trip WHERE trip.employee LIKE '%[$delete_id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip);";
        $query .= "DELETE FROM `employee` WHERE `id` = '$delete_id';";
        $query .= "DELETE FROM `trip` WHERE employee LIKE '%[$delete_id]|[%%]|[%%]%';";
...
if (isset($_GET['id'])) {
    ...
    $result_employee = mysqli_query($con, $query);
    $query = "SELECT * FROM `trip` WHERE employee LIKE '%[$id]|[%%]|[%%]%';";
    $result_trip = mysqli_query($con, $query);
    $query = "SELECT coupon.* FROM coupon INNER JOIN trip WHERE trip.employee LIKE '%[$id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip) GROUP BY coupon.id;";
    $result_coupon = mysqli_query($con, $query);
    if ($result_employee && $result_trip && $result_coupon) {
        ...
        <div class='modal-content'>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class='row d-flex overflow-auto' style='flex-wrap: nowrap;'>
                                            <?php
                                            if ($employee['profile_image'] != "" && file_exists("../../../Images/" . $employee['profile_image'])) {
                                                ?>
                                                <div class='col-12 d-flex align-items-center py-3'>
                                                    <img src='../Images/<?php echo $employee['profile_image']; ?>' alt=''
                                    ...
                                    <td>
                                        <?php echo explode('|', $employee['emp_type'])[1]; ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <?php echo $employee['id_type']; ?>
                                    </td>
                                    <td>
                                        <?php echo $employee['id_number']; ?>
                                    </td>
                                    <td>
                                        <?php echo substr(explode("|", $employee['country'])[1], 1, -1); ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class='row d-flex overflow-auto' style='flex-wrap: nowrap;'>
                                            <?php
                                            if ($employee['id_image'] != "" && file_exists("../../../Images/" . $employee['id_image'])) {
                                                ?>
                                                <div class='col-12 d-flex align-items-center py-3'>
                                                    <img src='../Images/<?php echo $employee['id_image']; ?>' alt='' class='w-100'
            ...
            <div class='modal-footer'>
                ...
                <button type='button' class='btn btn-danger' data-dismiss='modal'
                    onclick='employee_delete_data(<?php echo $employee['id']; ?>)'>Confirm</button>
            </div>
        </div>
        <?php
    }

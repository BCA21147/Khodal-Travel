...
if (isset($_POST['delete_id'])) {
    ...
    $query = "DELETE coupon.* FROM coupon INNER JOIN trip WHERE trip.schedule_time LIKE '%[$delete_id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip);";
    $query .= "DELETE FROM `schedule` WHERE `id` = '$delete_id';";
    $query .= "DELETE FROM `trip` WHERE schedule_time LIKE '%[$delete_id]|[%%]%';";
...
if (isset($_GET['id'])) {
    ...
    $result_schedule = mysqli_query($con, $query);
    $query = "SELECT * FROM `trip` WHERE schedule_time LIKE '%[$id]|[%%]%';";
    $result_trip = mysqli_query($con, $query);
    $query = "SELECT coupon.* FROM coupon INNER JOIN trip WHERE trip.schedule_time LIKE '%[$id]|[%%]%' AND LOCATE(CONCAT('[',trip.id,']'),trip) GROUP BY coupon.id;";
    $result_coupon = mysqli_query($con, $query);
    if ($result_schedule && $result_trip && $result_coupon) {
        ...
        <div class='modal-content'>
                        ...
                        <table class='table table-hover table-bordered'>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <?php echo $schedule['start_time_12_hour']; ?>
                                    </td>
                                    <td>
                                        <?php echo $schedule['end_time_12_hour']; ?>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class='modal-footer'>
                ...
                <button type='button' class='btn btn-danger' data-dismiss='modal'
                    onclick='schedule_delete_data(<?php echo $schedule['id']; ?>)'>Confirm</button>
            </div>
        </div>
        <?php
    }
